<template>
  <div>
    ...
    <table v-else class="table">
      <thead>
        <tr><th>Назва</th><th>Ціна</th><th>Активний</th><th></th></tr>
      </thead>
      <tbody>
    ...
    <div v-if="modal" class="modal-overlay" @click.self="modal = false">
      <div class="modal">
        <h3 class="modal__title">{{ editing ? 'Редагувати' : 'Нова ціна' }}</h3>
        ...
        <div class="form__field"><label>Ціна (грн)</label><input v-model.number="form.price" type="number" min="0" /></div>
        <div class="form__field"><label>Тип</label><input v-model="form.type" type="text" placeholder="person / group / day" /></div>
...
const form    = ref({ title: '', description: '', price: null, type: '', is_active: true })

async function load() {
  loading.value = true
  try { const { data } = await axios.get('/api/v1/admin/prices'); items.value = data?.data ?? data }
  finally { loading.value = false }
}
onMounted(load)

function openCreate() { editing.value = null; form.value = { title: '', description: '', price: null, type: '', is_active: true }; modal.value = true }
...
async function save() {
  ...
  try {
    editing.value
      ? await axios.put(`/api/v1/admin/prices/${editing.value}`, form.value)
      : await axios.post('/api/v1/admin/prices', form.value)
...
async function remove(id) {
  ...
  await axios.delete(`/api/v1/admin/prices/${id}`); await load()
}
</script>
